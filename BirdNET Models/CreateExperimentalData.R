# ===============================================================
# Simple "Reverse" Grid Builder
# - 3,000 per class (Background, HW, SRKW, TKW)
# - SRKW/TKW base = 3,000 UNTYPED (provider-stratified)
# - Boosts: replace untyped with equal-per-calltype typed:
#     * 10% → 300 typed total per ecotype (equal split across repertoire)
#     * 30% → 900 typed total per ecotype (equal split; builds on 10%)
# - If a calltype lacks enough real examples, augment with time-shift
# - BG & HW fixed across all 9 experiments (provider-stratified)
# - Console plots only
# ===============================================================
...
# Config & Paths
# -----------------------------
in_csv  <- "C:\\Users\\kaity\\Documents\\GitHub\\Ecotype\\Experiments\\BirdnetOrganized\\DCLDE_train_parent.csv"
out_dir <- "C:\\Users\\kaity\\Documents\\GitHub\\Ecotype\\Experiments\\BirdnetOrganized\\BirdnetGrid"
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

N_PER_CLASS <- 3000L
KW_LABELS   <- c("SRKW","TKW")

EXCLUDE_CT <- c("Buzz","buzz","EL","Multiple","Rasp","Unk","Whistle","whistle/tone",
                "W","whistle","rasp","Ck","","Whup")
...
# Typed pool = known repertoire, filtered (no '?', not in EXCLUDE_CT)
D_kw_typed <- D %>%
  filter(Labels %in% KW_LABELS,
         !HasQ,
         !(CallType %in% EXCLUDE_CT))

# Helper: map raw CallType string to one of the repertoire tokens for the ecotype
map_calltypes <- function(df, ecotype) {
  toks <- kw_cats[[ecotype]]
  if (is.null(toks) || length(toks) == 0) return(df %>% mutate(CalltypeCategory = "None"))
  df %>%
    mutate(CalltypeCategory = {
      hit <- map_chr(CallType, function(ct) {
        m <- toks[str_detect(ct, fixed(toks))]
        if (length(m) == 0) "None" else m[1]
      })
      hit
    }) %>%
    filter(CalltypeCategory != "None")
}

TYPED_SRKW <- D_kw_typed %>% filter(Labels == "SRKW") %>% map_calltypes("SRKW")
TYPED_TKW  <- D_kw_typed %>% filter(Labels == "TKW")  %>% map_calltypes("TKW")

# Untyped pools = ecotype rows not in typed pool IDs
UNTYPED_SRKW <- D %>% filter(Labels == "SRKW", !(ID %in% TYPED_SRKW$ID))
UNTYPED_TKW  <- D %>% filter(Labels == "TKW",  !(ID %in% TYPED_TKW$ID))

# -----------------------------
# Provider-stratified sampler
# -----------------------------
stratified_sample_n <- function(df, group_col = "Provider", n_total) {
  if (nrow(df) == 0 || n_total <= 0) return(df[0, ])
  df2 <- df %>% mutate(.grp = ifelse(is.na(.data[[group_col]]), "__NA__", as.character(.data[[group_col]])))
  gtab <- df2 %>% count(.grp, name = "n")
  if (sum(gtab$n) < n_total) {
    return(df2 %>% slice_sample(n = n_total, replace = TRUE) %>% select(-.grp))
  }
  props <- gtab %>% mutate(target = pmax(0L, round(n_total * n / sum(n))))
  drift <- n_total - sum(props$target)
  if (drift != 0) {
    ord <- order(-props$n); ix <- rep(ord, length.out = abs(drift))
  ...
  have <- nrow(sampled)
  if (have == n_total) return(sampled %>% select(-any_of(c(".grp","n","target"))))
  need <- n_total - have
  remaining <- df2 %>% anti_join(sampled %>% select(ID), by = "ID")
  cap <- remaining %>% count(.grp, name = "cap")
  cap <- cap %>% mutate(take = pmax(0L, floor(need * cap / sum(cap))))
  drift2 <- need - sum(cap$take)
  if (drift2 != 0) {
    ord2 <- order(-cap$cap); ix2 <- rep(ord2, length.out = abs(drift2))
  ...
  if (nrow(sampled) > n_total) sampled <- dplyr::slice_sample(sampled, n = n_total)
  sampled %>% select(-any_of(c(".grp","n","target","cap","take")))
}
...
# Build regex of ALL known repertoire tokens (exact) + a generic S##/T## safety net
kw_tokens_all <- c(kw_cats$SRKW, kw_cats$TKW)
kw_exact_regex <- paste0("(", paste(stringr::str_c("\\b", kw_tokens_all, "\\b"), collapse = "|"), ")")
kw_generic_regex <- "(\\bS\\d{2}\\b|\\bT\\d{2}\\b)"
kw_any_regex <- paste0("(", kw_exact_regex, "|", kw_generic_regex, ")")

# Background from AB + UndBio only, explicitly not SRKW/TKW, and with NO KW-like token in CallType
BG_pool <- D %>%
  dplyr::filter(
    Labels == "Background",
  ...
  dplyr::filter(is.na(CallType) | CallType == "" | !stringr::str_detect(CallType, kw_any_regex))

stopifnot(nrow(BG_pool) >= N_PER_CLASS)

BG_FIXED <- stratified_sample_n(BG_pool, "Provider", N_PER_CLASS) %>%
  dplyr::mutate(Labels = "Background", CalltypeCategory = "None")

# HW as before, but make sure CalltypeCategory is "None"
HW_pool <- D %>% dplyr::filter(Labels == "HW")
stopifnot(nrow(HW_pool) >= N_PER_CLASS)

HW_FIXED <- stratified_sample_n(HW_pool, "Provider", N_PER_CLASS) %>%
  dplyr::mutate(Labels = "HW", CalltypeCategory = "None")

# --- Sanity: no KW tokens in BG/HW ---
kw_in_bg <- BG_FIXED %>% dplyr::filter(!is.na(CallType) & CallType != "" & stringr::str_detect(CallType, kw_any_regex))
kw_in_hw <- HW_FIXED %>% dplyr::filter(!is.na(CallType) & CallType != "" & stringr::str_detect(CallType, kw_any_regex))
stopifnot(nrow(kw_in_bg) == 0, nrow(kw_in_hw) == 0)


# -----------------------------
# Augmentation (unique IDs across run)
# -----------------------------
.AUG_ID_BASE <- max(D$ID, na.rm = TRUE)
.AUG_ID_INC  <- 0L
augment_rows <- function(df, need) {
  if (need <= 0) return(df[0, ])
  base <- df %>% slice_sample(n = need, replace = TRUE)
...
equal_split_targets <- function(total_needed, tokens) {
  ...
  tgt <- rep(base_q, K)
  if (rem_q > 0) tgt[seq_len(rem_q)] <- tgt[seq_len(rem_q)] + 1L
  tibble::tibble(CalltypeCategory = tokens, target_n = tgt)
}

per_ct_draw <- function(ecotype, ct, need, typed_pool_all) {
  if (need <= 0) return(typed_pool_all[0, ])
  # real examples for this CT
  avail <- typed_pool_all %>% filter(CalltypeCategory == ct)
  take_real <- min(nrow(avail), need)
  real_rows <- if (take_real > 0) stratified_sample_n(avail, "Provider", take_real) else avail[0, ]
  short <- need - take_real
  if (short > 0) {
    if (nrow(avail) > 0) {
      aug <- augment_rows(avail, short); aug$AugFromCT <- ct
    } else {
      # ZERO seed: borrow from largest CT for this ecotype
...
# Replace untyped rows with typed (equal per CT); nesting-friendly
replace_with_equal_ct <- function(prev_df, ecotype, typed_pool_all, pct) {
  target_total <- round(pct * N_PER_CLASS)          # e.g., 300 or 900
  # how many typed do we already have?
  ...
  tokens <- kw_cats[[ecotype]]
  # current per-CT counts (in prev_df)
  curr_ct <- prev_df %>%
    filter(Labels == ecotype, CalltypeCategory != "None") %>%
    count(CalltypeCategory, name = "n") %>%
    right_join(tibble::tibble(CalltypeCategory = tokens), by = "CalltypeCategory") %>%
    mutate(n = replace_na(n, 0L)) %>% arrange(CalltypeCategory)

  # desired final per-CT totals for this pct
  targets <- equal_split_targets(target_total, tokens)

  # compute shortfalls
  need_tbl <- curr_ct %>% left_join(targets, by = "CalltypeCategory") %>%
    mutate(need = pmax(0L, target_n - n))

  add_list <- map2(need_tbl$CalltypeCategory, need_tbl$need,
                   ~ per_ct_draw(ecotype, .x, .y, typed_pool_all))
  add_typed <- bind_rows(add_list)

  # replace that many currently-untyped rows
  replace_n <- nrow(add_typed)
  untyped_ids <- prev_df$ID[prev_df$CalltypeCategory == "None" | is.na(prev_df$CalltypeCategory)]
  if (length(untyped_ids) < replace_n) {
    # should not usually happen, but allow fallback
    extra <- replace_n - length(untyped_ids)
    typed_ids <- prev_df$ID[prev_df$CalltypeCategory != "None" & !is.na(prev_df$CalltypeCategory)]
    replace_ids <- c(untyped_ids, sample(typed_ids, extra))
  } else {
    replace_ids <- sample(untyped_ids, replace_n)
  }
  keep_df <- prev_df %>% filter(!(ID %in% replace_ids))
  out <- bind_rows(keep_df, add_typed) %>%
    mutate(CalltypeCategory = ifelse(is.na(CalltypeCategory) | CalltypeCategory == "", "None", CalltypeCategory)) %>%
    arrange(ID)
  out
}
...
# -----------------------------
# Console diagnostics (stacked bars)
# -----------------------------
plot_stacked_by_label_counts <- function(df, title_suffix) {
  df2 <- df %>% mutate(CalltypeCategory = ifelse(is.na(CalltypeCategory)|CalltypeCategory=="","None",CalltypeCategory))
...
plot_stacked_by_label_props <- function(df, title_suffix) {
  ...
  df2 %>%
    count(Labels, CalltypeCategory, name = "n") %>%
    group_by(Labels) %>% mutate(prop = n / sum(n)) %>% ungroup() %>%
...
for (nm in names(grid)) {
  ...
  # write CSV
  #write_csv(final_df, file.path(out_dir, paste0("Grid_", nm, ".csv")))
  message("Wrote Grid_", nm, ".csv")
