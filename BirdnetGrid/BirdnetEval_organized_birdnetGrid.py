...
from sklearn.metrics import matthews_corrcoef
# ------------------------------------------------------------
# GLOBAL FONT SETTINGS FOR ALL TABLES / LEGENDS / FIGURES
# ------------------------------------------------------------
import matplotlib as mpl
import seaborn as sns

# --- Choose a consistent font family for the entire paper ---
# Recommended: Arial (journal-safe), or DejaVu Sans if Arial isn’t installed.
mpl.rcParams.update({
    "font.family": "sans-serif",
    "font.sans-serif": ["Arial"],   # or ["DejaVu Sans"], ["Helvetica"], etc.
...
# --- Make seaborn match the same font / size rules ---
sns.set_theme(
    context="paper",
    style="white",
    font="sans-serif",
...
def _plot_pr_on_ax(metrics_list, common_labels, target_class, ax, color_map):
    """Internal helper: plot PR curves for one class on a provided Axes using shared labels."""
    for metrics, label in zip(metrics_list, common_labels):
        pr = metrics.get('pr_data', {}).get(target_class, None)
        if pr is None:
            continue
        ax.plot(pr['recall'], pr['precision'],
                label=label, lw=1.5, alpha=0.9, color=color_map[label])
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.grid(True, linewidth=0.6, alpha=0.4)

def plot_pr_grid_3x2(
    dclde_metrics_list, 
    malahat_metrics_list, 
    common_model_labels,               # e.g., ["BirdNET_01", ..., "BirdNET_09"]
    classes=('SRKW','TKW','HW'),
    panel_letters=True,
    figsize=(12, 12),
    savepath=None,
    ...
    """
    Create a 3x2 grid of precision–recall curves:
      rows  -> classes (SRKW, TKW, HW)
      cols  -> datasets (DCLDE, Malahat)

    Parameters
    ----------
    dclde_metrics_list : list[dict]
        Metrics dicts for DCLDE (aligned to common_model_labels by index).
    malahat_metrics_list : list[dict]
        Metrics dicts for Malahat (aligned to common_model_labels by index).
    common_model_labels : list[str]
        Shared labels applied to BOTH datasets (e.g., BirdNET_01..09).
    classes : tuple[str]
        Classes to plot in row order.
    panel_letters : bool
        If True, annotate panels (A–F).
    figsize : tuple
        Figure size in inches.
    savepath : str or None
        If set, save the figure to this path.
    dpi : int
        Save resolution (ignored if savepath is None).
    """
    if len(dclde_metrics_list) != len(malahat_metrics_list) or \
       len(dclde_metrics_list) != len(common_model_labels):
        raise ValueError("dclde_metrics_list, malahat_metrics_list, and common_model_labels must have equal length.")

    # Consistent color map keyed by shared labels
    default_cycle = plt.rcParams['axes.prop_cycle'].by_key()['color']
    while len(default_cycle) < len(common_model_labels):
        default_cycle += default_cycle
    color_map = {lab: default_cycle[i] for i, lab in enumerate(common_model_labels)}

    fig, axes = plt.subplots(nrows=3, ncols=2, figsize=figsize, sharex=True, sharey=True)
    col_titles = ('DCLDE', 'Malahat')

    # Plot each cell
    for r, cls in enumerate(classes):
        # DCLDE (left)
        ax_left = axes[r, 0]
        _plot_pr_on_ax(dclde_metrics_list, common_model_labels, cls, ax_left, color_map)
        if r == 0:
            ax_left.set_title(col_titles[0], fontsize=12, pad=8)

        # Malahat (right)
        ax_right = axes[r, 1]
        _plot_pr_on_ax(malahat_metrics_list, common_model_labels, cls, ax_right, color_map)
        if r == 0:
            ax_right.set_title(col_titles[1], fontsize=12, pad=8)

    # Axis labels (outer only)
    for r, cls in enumerate(classes):
        axes[r, 0].set_ylabel(f'{cls}\nPrecision', fontsize=11)
    for c in range(2):
        axes[2, c].set_xlabel('Recall', fontsize=11)
    ...
    if panel_letters:
        ...
        for r in range(3):
            for c in range(2):
                axes[r, c].text(0.02, 0.98, letters[k],
                                transform=axes[r, c].transAxes,
                                ha='left', va='top', fontsize=12, fontweight='bold')
                k += 1

    # Single legend outside
    legend_handles = [Line2D([0], [0], color=color_map[lab], lw=2, label=lab)
                      for lab in common_model_labels]
...
if __name__ == "__main__":
    ...
    # =========================
    # Clean BirdNET 01–09 block
    # =========================
    from pathlib import Path
    
    GRID = Path(r"C:\Users\kaity\Documents\GitHub\Ecotype\Experiments\BirdnetOrganized\BirdnetGrid")
    
    # ---- model configs (8 kHz cutoff variants you provided) ----
    model_config_01 = {
        "model_path": str(GRID / "birdnet01" / "birdnet01_8khz_cutoff.tflite"),
        "label_path": str(GRID / "birdnet01" / "birdnet01_8khz_cutoff_Labels.txt"),
    }
    model_config_02 = {
        "model_path": str(GRID / "birdnet02" / "birdnet02_8khz_cutoff.tflite"),
        "label_path": str(GRID / "birdnet02" / "birdnet02_8khz_cutoff_Labels.txt"),
    }
    model_config_03 = {
        "model_path": str(GRID / "birdnet03" / "birdnet03_8khz_cutoff.tflite"),
        "label_path": str(GRID / "birdnet03" / "birdnet03_8khz_cutoff_Labels.txt"),
    }
    model_config_04 = {
        "model_path": str(GRID / "birdnet04" / "birdnet04_8khz_cutoff.tflite"),
        "label_path": str(GRID / "birdnet04" / "birdnet04_8khz_cutoff_Labels.txt"),
    }
    model_config_05 = {
        "model_path": str(GRID / "birdnet05" / "birdnet05_8khz_cutoff.tflite"),
        "label_path": str(GRID / "birdnet05" / "birdnet05_8khz_cutoff_Labels.txt"),
    }
    model_config_06 = {
        "model_path": str(GRID / "birdnet06" / "birdnet06_8khz_cutoff.tflite"),
        "label_path": str(GRID / "birdnet06" / "birdnet06_8khz_cutoff_Labels.txt"),
    }
    model_config_07 = {
        "model_path": str(GRID / "birdnet07" / "birdnet07_8khz_cutoff.tflite"),
        "label_path": str(GRID / "birdnet07" / "birdnet07_8khz_cutoff_Labels.txt"),
    }
    model_config_08 = {
        "model_path": str(GRID / "birdnet08" / "birdnet08_8khz_cutoff.tflite"),
        "label_path": str(GRID / "birdnet08" / "birdnet08_8khz_cutoff_Labels.txt"),
    }
    model_config_09 = {
        "model_path": str(GRID / "birdnet09" / "birdnet09_8khz_cutoff.tflite"),
        "label_path": str(GRID / "birdnet09" / "birdnet09_8khz_cutoff_Labels.txt"),
    }
    
    # ---- common helpers to keep paths tidy ----
    def _csv(name: str) -> str:
        return str(GRID / name)
    
    def _load_or_process(csv_path: Path, model_cfg: dict, folders: dict,
                         classes=None):
        """Load eval CSV if present; else run processing and write it."""
        csv_path = Path(csv_path)
        try:
            return pd.read_csv(csv_path)
        except Exception:
            df = process_multiple_datasets(
                model_cfg["model_path"],
                model_cfg["label_path"],
                folders,
                output_csv=str(csv_path),   # ensure BirdNet writes to the same file
                classes=classes if classes is not None else ['SRKW','OKW','HW','TKW'],
                thresh=0
            )
    ...
    # =====================
    # BirdNET 01 (8 kHz cut)
    # =====================
    dclde_csv_01   = GRID / "birdnet01_DCLDE_eval.csv"
    malahat_csv_01 = GRID / "birdnet01_Malahat_eval.csv"
    
    eval_dclde_birdnet_01   = _load_or_process(dclde_csv_01,   model_config_01, dclde_folders)
    eval_malahat_birdnet_01 = _load_or_process(malahat_csv_01, model_config_01, malahat_folders)
    
    hw_cutoff   = compute_threshold(eval_dclde_birdnet_01[eval_dclde_birdnet_01['Truth']=="HW"],   score_column="HW",   class_column="Predicted Class", title_suffix="Humpback")
    tkw_cutoff  = compute_threshold(eval_dclde_birdnet_01[eval_dclde_birdnet_01['Truth']=="TKW"],  score_column="TKW",  class_column="Predicted Class", title_suffix="TKW")
    srkw_cutoff = compute_threshold(eval_dclde_birdnet_01[eval_dclde_birdnet_01['Truth']=="SRKW"], score_column="SRKW", class_column="Predicted Class", title_suffix="SRKW")
    custom_thresholds_dclde = {"HW": hw_cutoff, "TKW": tkw_cutoff, "SRKW": srkw_cutoff}
    ...
    custom_thresholds_malahat = {"HW": hw_cutoff_m, "TKW": tkw_cutoff_m, "SRKW": srkw_cutoff_m}
    
    metrics_DCLDE_01   = evaluate_model(eval_dclde_birdnet_01,   custom_thresholds=custom_thresholds_dclde,   pr_title="DCLDE Precision–Recall Curve 01",   roc_title="DCLDE ROC Curve 01")
    metrics_malahat_01 = evaluate_model(eval_malahat_birdnet_01, custom_thresholds=custom_thresholds_dclde,   pr_title="Malahat Precision–Recall Curve 01",  roc_title="Malahat ROC Curve 01")
    
    # =====================
    # BirdNET 02 (8 kHz cut)
    # =====================
    dclde_csv_02   = GRID / "birdnet02_DCLDE_eval.csv"
    malahat_csv_02 = GRID / "birdnet02_Malahat_eval.csv"
    
    # FIX: use model_config_02 here (was model_config_01 before)
    eval_dclde_birdnet_02   = _load_or_process(dclde_csv_02,   model_config_02, dclde_folders)
    eval_malahat_birdnet_02 = _load_or_process(malahat_csv_02, model_config_02, malahat_folders)
    
    hw_cutoff   = compute_threshold(eval_dclde_birdnet_02[eval_dclde_birdnet_02['Truth']=="HW"],   score_column="HW",   class_column="Predicted Class", title_suffix="Humpback")
    tkw_cutoff  = compute_threshold(eval_dclde_birdnet_02[eval_dclde_birdnet_02['Truth']=="TKW"],  score_column="TKW",  class_column="Predicted Class", title_suffix="TKW")
    srkw_cutoff = compute_threshold(eval_dclde_birdnet_02[eval_dclde_birdnet_02['Truth']=="SRKW"], score_column="SRKW", class_column="Predicted Class", title_suffix="SRKW")
    custom_thresholds_dclde = {"HW": hw_cutoff, "TKW": tkw_cutoff, "SRKW": srkw_cutoff}
    ...
    metrics_DCLDE_02   = evaluate_model(eval_dclde_birdnet_02,   custom_thresholds=custom_thresholds_dclde,   pr_title="DCLDE Precision–Recall Curve 02",   roc_title="DCLDE ROC Curve 02")
    metrics_malahat_02 = evaluate_model(eval_malahat_birdnet_02, custom_thresholds=custom_thresholds_dclde,   pr_title="Malahat Precision–Recall Curve 02",  roc_title="Malahat ROC Curve 02")
    
    # =====================
    # BirdNET 03 (8 kHz cut)
    # =====================
    dclde_csv_03   = GRID / "birdnet03_DCLDE_eval.csv"
    malahat_csv_03 = GRID / "birdnet03_Malahat_eval.csv"
    
    eval_dclde_birdnet_03   = _load_or_process(dclde_csv_03,   model_config_03, dclde_folders)
    eval_malahat_birdnet_03 = _load_or_process(malahat_csv_03, model_config_03, malahat_folders, classes=['SRKW','HW','TKW'])
    
    hw_cutoff   = compute_threshold(eval_dclde_birdnet_03[eval_dclde_birdnet_03['Truth']=="HW"],   score_column="HW",   class_column="Predicted Class", title_suffix="Humpback")
    tkw_cutoff  = compute_threshold(eval_dclde_birdnet_03[eval_dclde_birdnet_03['Truth']=="TKW"],  score_column="TKW",  class_column="Predicted Class", title_suffix="TKW")
    srkw_cutoff = compute_threshold(eval_dclde_birdnet_03[eval_dclde_birdnet_03['Truth']=="SRKW"], score_column="SRKW", class_column="Predicted Class", title_suffix="SRKW")
    custom_thresholds_dclde = {"HW": hw_cutoff, "TKW": tkw_cutoff, "SRKW": srkw_cutoff}
    ...
    metrics_DCLDE_03   = evaluate_model(eval_dclde_birdnet_03,   custom_thresholds=custom_thresholds_dclde,   pr_title="DCLDE Precision–Recall Curve 03",   roc_title="DCLDE ROC Curve 03")
    metrics_malahat_03 = evaluate_model(eval_malahat_birdnet_03, custom_thresholds=custom_thresholds_dclde,   pr_title="Malahat Precision–Recall Curve 03",  roc_title="Malahat ROC Curve 03")
    
    # =====================
    # BirdNET 04 (8 kHz cut)
    # =====================
    dclde_csv_04   = GRID / "birdnet04_DCLDE_eval.csv"
    malahat_csv_04 = GRID / "birdnet04_Malahat_eval.csv"
    
    eval_dclde_birdnet_04   = _load_or_process(dclde_csv_04,   model_config_04, dclde_folders)
    eval_malahat_birdnet_04 = _load_or_process(malahat_csv_04, model_config_04, malahat_folders, classes=['SRKW','HW','TKW'])
    
    hw_cutoff   = compute_threshold(eval_dclde_birdnet_04[eval_dclde_birdnet_04['Truth']=="HW"],   score_column="HW",   class_column="Predicted Class", title_suffix="Humpback")
    tkw_cutoff  = compute_threshold(eval_dclde_birdnet_04[eval_dclde_birdnet_04['Truth']=="TKW"],  score_column="TKW",  class_column="Predicted Class", title_suffix="TKW")
    srkw_cutoff = compute_threshold(eval_dclde_birdnet_04[eval_dclde_birdnet_04['Truth']=="SRKW"], score_column="SRKW", class_column="Predicted Class", title_suffix="SRKW")
    custom_thresholds_dclde = {"HW": hw_cutoff, "TKW": tkw_cutoff, "SRKW": srkw_cutoff}
    ...
    metrics_DCLDE_04   = evaluate_model(eval_dclde_birdnet_04,   custom_thresholds=custom_thresholds_dclde,   pr_title="DCLDE Precision–Recall Curve 04",   roc_title="DCLDE ROC Curve 04")
    metrics_malahat_04 = evaluate_model(eval_malahat_birdnet_04, custom_thresholds=custom_thresholds_dclde,   pr_title="Malahat Precision–Recall Curve 04",  roc_title="Malahat ROC Curve 04")
    
    # =====================
    # BirdNET 05 (8 kHz cut)
    # =====================
    dclde_csv_05   = GRID / "birdnet05_DCLDE_eval.csv"
    malahat_csv_05 = GRID / "birdnet05_Malahat_eval.csv"
    
    eval_dclde_birdnet_05   = _load_or_process(dclde_csv_05,   model_config_05, dclde_folders)
    eval_malahat_birdnet_05 = _load_or_process(malahat_csv_05, model_config_05, malahat_folders, classes=['SRKW','HW','TKW'])
    
    hw_cutoff   = compute_threshold(eval_dclde_birdnet_05[eval_dclde_birdnet_05['Truth']=="HW"],   score_column="HW",   class_column="Predicted Class", title_suffix="Humpback")
    tkw_cutoff  = compute_threshold(eval_dclde_birdnet_05[eval_dclde_birdnet_05['Truth']=="TKW"],  score_column="TKW",  class_column="Predicted Class", title_suffix="TKW")
    srkw_cutoff = compute_threshold(eval_dclde_birdnet_05[eval_dclde_birdnet_05['Truth']=="SRKW"], score_column="SRKW", class_column="Predicted Class", title_suffix="SRKW")
    custom_thresholds_dclde = {"HW": hw_cutoff, "TKW": tkw_cutoff, "SRKW": srkw_cutoff}
    ...
    metrics_DCLDE_05   = evaluate_model(eval_dclde_birdnet_05,   custom_thresholds=custom_thresholds_dclde,   pr_title="DCLDE Precision–Recall Curve 05",   roc_title="DCLDE ROC Curve 05")
    metrics_malahat_05 = evaluate_model(eval_malahat_birdnet_05, custom_thresholds=custom_thresholds_dclde,   pr_title="Malahat Precision–Recall Curve 05",  roc_title="Malahat ROC Curve 05")
    
    # =====================
    # BirdNET 06 (8 kHz cut)
    # =====================
    dclde_csv_06   = GRID / "birdnet06_DCLDE_eval.csv"
    malahat_csv_06 = GRID / "birdnet06_Malahat_eval.csv"
    
    eval_dclde_birdnet_06   = _load_or_process(dclde_csv_06,   model_config_06, dclde_folders)
    eval_malahat_birdnet_06 = _load_or_process(malahat_csv_06, model_config_06, malahat_folders, classes=['SRKW','HW','TKW'])
    
    hw_cutoff   = compute_threshold(eval_dclde_birdnet_06[eval_dclde_birdnet_06['Truth']=="HW"],   score_column="HW",   class_column="Predicted Class", title_suffix="Humpback")
    tkw_cutoff  = compute_threshold(eval_dclde_birdnet_06[eval_dclde_birdnet_06['Truth']=="TKW"],  score_column="TKW",  class_column="Predicted Class", title_suffix="TKW")
    srkw_cutoff = compute_threshold(eval_dclde_birdnet_06[eval_dclde_birdnet_06['Truth']=="SRKW"], score_column="SRKW", class_column="Predicted Class", title_suffix="SRKW")
    custom_thresholds_dclde = {"HW": hw_cutoff, "TKW": tkw_cutoff, "SRKW": srkw_cutoff}
    ...
    metrics_DCLDE_06   = evaluate_model(eval_dclde_birdnet_06,   custom_thresholds=custom_thresholds_dclde,   pr_title="DCLDE Precision–Recall Curve 06",   roc_title="DCLDE ROC Curve 06")
    metrics_malahat_06 = evaluate_model(eval_malahat_birdnet_06, custom_thresholds=custom_thresholds_dclde,   pr_title="Malahat Precision–Recall Curve 06",  roc_title="Malahat ROC Curve 06")
    
    # =====================
    # BirdNET 07 (8 kHz cut)
    # =====================
    dclde_csv_07   = GRID / "birdnet07_DCLDE_eval.csv"
    malahat_csv_07 = GRID / "birdnet07_Malahat_eval.csv"
    
    eval_dclde_birdnet_07   = _load_or_process(dclde_csv_07,   model_config_07, dclde_folders)
    eval_malahat_birdnet_07 = _load_or_process(malahat_csv_07, model_config_07, malahat_folders, classes=['SRKW','HW','TKW'])
    
    hw_cutoff   = compute_threshold(eval_dclde_birdnet_07[eval_dclde_birdnet_07['Truth']=="HW"],   score_column="HW",   class_column="Predicted Class", title_suffix="Humpback")
    tkw_cutoff  = compute_threshold(eval_dclde_birdnet_07[eval_dclde_birdnet_07['Truth']=="TKW"],  score_column="TKW",  class_column="Predicted Class", title_suffix="TKW")
    srkw_cutoff = compute_threshold(eval_dclde_birdnet_07[eval_dclde_birdnet_07['Truth']=="SRKW"], score_column="SRKW", class_column="Predicted Class", title_suffix="SRKW")
    custom_thresholds_dclde = {"HW": hw_cutoff, "TKW": tkw_cutoff, "SRKW": srkw_cutoff}
    ...
    metrics_DCLDE_07   = evaluate_model(eval_dclde_birdnet_07,   custom_thresholds=custom_thresholds_dclde,   pr_title="DCLDE Precision–Recall Curve 07",   roc_title="DCLDE ROC Curve 07")
    metrics_malahat_07 = evaluate_model(eval_malahat_birdnet_07, custom_thresholds=custom_thresholds_dclde,   pr_title="Malahat Precision–Recall Curve 07",  roc_title="Malahat ROC Curve 07")
    
    # =====================
    # BirdNET 08 (8 kHz cut)
    # =====================
    dclde_csv_08   = GRID / "birdnet08_DCLDE_eval.csv"
    malahat_csv_08 = GRID / "birdnet08_Malahat_eval.csv"
    
    eval_dclde_birdnet_08   = _load_or_process(dclde_csv_08,   model_config_08, dclde_folders)
    eval_malahat_birdnet_08 = _load_or_process(malahat_csv_08, model_config_08, malahat_folders, classes=['SRKW','HW','TKW'])
    
    hw_cutoff   = compute_threshold(eval_dclde_birdnet_08[eval_dclde_birdnet_08['Truth']=="HW"],   score_column="HW",   class_column="Predicted Class", title_suffix="Humpback")
    tkw_cutoff  = compute_threshold(eval_dclde_birdnet_08[eval_dclde_birdnet_08['Truth']=="TKW"],  score_column="TKW",  class_column="Predicted Class", title_suffix="TKW")
    srkw_cutoff = compute_threshold(eval_dclde_birdnet_08[eval_dclde_birdnet_08['Truth']=="SRKW"], score_column="SRKW", class_column="Predicted Class", title_suffix="SRKW")
    custom_thresholds_dclde = {"HW": hw_cutoff, "TKW": tkw_cutoff, "SRKW": srkw_cutoff}
    
    hw_cutoff_m   = compute_threshold(eval_malahat_birdnet_08[eval_malahat_birdnet_08['Truth']=="HW"],   class_column="Predicted Class", score_column="HW",   title_suffix="Malahat HW")
    tkw_cutoff_m  = compute_threshold(eval_malahat_birdnet_08[eval_malahat_birdnet_08['Truth']=="TKW"],  class_column="Predicted Class", score_column="TKW",  title_suffix="Malahat TKW")
    srkw_cutoff_m = compute_threshold(eval_malahat_birdnet_08[eval_malahat_birdnet_08['Truth']=="SRKW"], class_column="Predicted Class", score_column="SRKW", title_suffix="Malahat SRKW")
    custom_thresholds_malahat = {"HW": hw_cutoff_m, "TKW": tkw_cutoff_m, "SRKW": srkw_cutoff_m}
    
    # FIX: thresholds were swapped before — keep them aligned
    metrics_DCLDE_08   = evaluate_model(eval_dclde_birdnet_08,   custom_thresholds=custom_thresholds_dclde,   pr_title="DCLDE Precision–Recall Curve 08",   roc_title="DCLDE ROC Curve 08")
    metrics_malahat_08 = evaluate_model(eval_malahat_birdnet_08, custom_thresholds=custom_thresholds_dclde,   pr_title="Malahat Precision–Recall Curve 08",  roc_title="Malahat ROC Curve 08")
    
    # =====================
    # BirdNET 09 (8 kHz cut)
    # =====================
    dclde_csv_09   = GRID / "birdnet09_DCLDE_eval.csv"
    malahat_csv_09 = GRID / "birdnet09_Malahat_eval.csv"
    
    eval_dclde_birdnet_09   = _load_or_process(dclde_csv_09,   model_config_09, dclde_folders)
    eval_malahat_birdnet_09 = _load_or_process(malahat_csv_09, model_config_09, malahat_folders, classes=['SRKW','HW','TKW'])
    
    hw_cutoff   = compute_threshold(eval_dclde_birdnet_09[eval_dclde_birdnet_09['Truth']=="HW"],   score_column="HW",   class_column="Predicted Class", title_suffix="Humpback")
    tkw_cutoff  = compute_threshold(eval_dclde_birdnet_09[eval_dclde_birdnet_09['Truth']=="TKW"],  score_column="TKW",  class_column="Predicted Class", title_suffix="TKW")
    srkw_cutoff = compute_threshold(eval_dclde_birdnet_09[eval_dclde_birdnet_09['Truth']=="SRKW"], score_column="SRKW", class_column="Predicted Class", title_suffix="SRKW")
    custom_thresholds_dclde = {"HW": hw_cutoff, "TKW": tkw_cutoff, "SRKW": srkw_cutoff}
    
    hw_cutoff_m   = compute_threshold(eval_malahat_birdnet_09[eval_malahat_birdnet_09['Truth']=="HW"],   class_column="Predicted Class", score_column="HW",   title_suffix="Malahat HW")
    tkw_cutoff_m  = compute_threshold(eval_malahat_birdnet_09[eval_malahat_birdnet_09['Truth']=="TKW"],  class_column="Predicted Class", score_column="TKW",  title_suffix="Malahat TKW")
    srkw_cutoff_m = compute_threshold(eval_malahat_birdnet_09[eval_malahat_birdnet_09['Truth']=="SRKW"], class_column="Predicted Class", score_column="SRKW", title_suffix="Malahat SRKW")
    custom_thresholds_malahat = {"HW": hw_cutoff_m, "TKW": tkw_cutoff_m, "SRKW": srkw_cutoff_m}
    
    metrics_DCLDE_09   = evaluate_model(eval_dclde_birdnet_09,   custom_thresholds=custom_thresholds_dclde,   pr_title="DCLDE Precision–Recall Curve 09",   roc_title="DCLDE ROC Curve 09")
    metrics_malahat_09 = evaluate_model(eval_malahat_birdnet_09, custom_thresholds=custom_thresholds_dclde,   pr_title="Malahat Precision–Recall Curve 09",  roc_title="Malahat ROC Curve 09")
...
#%% Plot PR curves single file for publication

# Build the 3×2 figure
common_labels = [f"BirdNET_{i:02d}" for i in range(1, 10)]

fig, axes = plot_pr_grid_3x2(
    dclde_metrics_list=[metrics_DCLDE_01, metrics_DCLDE_02, metrics_DCLDE_03,
                        metrics_DCLDE_04, metrics_DCLDE_05, metrics_DCLDE_06,
                        metrics_DCLDE_07, metrics_DCLDE_08, metrics_DCLDE_09],
    malahat_metrics_list=[metrics_malahat_01, metrics_malahat_02, metrics_malahat_03,
                          metrics_malahat_04, metrics_malahat_05, metrics_malahat_06,
                          metrics_malahat_07, metrics_malahat_08, metrics_malahat_09],
    common_model_labels=common_labels,
    classes=('SRKW','TKW','HW'),
    panel_letters=True,
    figsize=(12, 12),
    savepath='PR_grid_3x2_600dpi.pdf',
    dpi=600
)
...
def plot_mcc_at_p90_from_metrics(
    ...
    ax.set_ylabel("SRKW recall at P90 Threshold")
    ax.set_xlabel("TKW recall at P90 Threshold")
    ax.set_title(title)
...
#%%  Figure pannel
# =======================
# MCC grid (1x2) with a rock-solid “legend” for MCC size/decision
# - Uses an INSET PANEL (not plt.legend) on Panel A, so the title never collides
#   and the labels are perfectly centered relative to the semicircles.
# - Panel B keeps a standard color legend for models.
# =======================
...
# ---------- validation ----------
def _validate_metrics_list(metrics_list, name):
    bad_idx, bad_types = [], []
    for i, m in enumerate(metrics_list):
        if not isinstance(m, Mapping):
            bad_idx.append(i); bad_types.append(type(m).__name__)
    if bad_idx:
        raise TypeError(
            f"{name} contains non-mapping entries at indices {bad_idx} with types {bad_types}. "
            "Each item must be a dict-like metrics object with keys "
            "'recall_at_p90', 'mcc_srkw_tkw', and 'mcc_srkw_hw'."
        )

# ---------- utilities ----------
def _collect_xy(metrics_list):
    xs, ys = [], []
    for i, met in enumerate(metrics_list):
        if not isinstance(met, Mapping):
            continue
        recs = met.get('recall_at_p90') or {}
        x, y = recs.get('TKW'), recs.get('SRKW')
        if x is None or y is None or not (np.isfinite(x) and np.isfinite(y)):
            continue
        xs.append(float(x)); ys.append(float(y))
    return np.array(xs, float), np.array(ys, float)

def _compute_limits(xs, ys, pad=0.03, min_span=0.15):
...
def _scale_vals(m, mode="linear", log_k=20.0):
    m = np.asarray(m, float)
    if mode == "linear": return m
    if mode == "sqrt":   return np.sqrt(m)
    return np.log1p(log_k * m) / np.log1p(log_k)

def _tint(color, factor=1.15):
    r,g,b = mcolors.to_rgb(color)
    return (1 - (1-r)/factor, 1 - (1-g)/factor, 1 - (1-b)/factor) if factor>=1 else (r*factor, g*factor, b*factor)

# ---------- panel plotter ----------
def _plot_panel_patterns(ax, metrics_list, model_labels, color_map,
                         title, size_method, log_k, r_min, r_max, radius_scale,
                         hatch_tkw='///', hatch_hw='xxx',
                         hatch_edge='0.35', hatch_lw=0.6,
                         axis_fs=12, title_fs=12):
    ...
    for lab, met in zip(model_labels, metrics_list):
        ...
        mL = float(np.nan_to_num(met.get('mcc_srkw_tkw', np.nan), nan=0.0))
        mR = float(np.nan_to_num(met.get('mcc_srkw_hw',  np.nan), nan=0.0))
        rL = (r_min + (r_max - r_min) * _scale_vals(mL, size_method, log_k)) * radius_scale
        rR = (r_min + (r_max - r_min) * _scale_vals(mR, size_method, log_k)) * radius_scale

        base = color_map[lab]
        ax.add_patch(Wedge((x, y), rL,  90, 270,
                           facecolor=_tint(base, 1.25), edgecolor=hatch_edge,
                           linewidth=0.6, alpha=0.97, hatch=hatch_tkw))
        ax.add_patch(Wedge((x, y), rR, -90,  90,
                           facecolor=_tint(base, 0.85), edgecolor=hatch_edge,
                           linewidth=0.6, alpha=0.97, hatch=hatch_hw))

    ax.set_xlabel("TKW Recall at P90 Threshold", fontsize=axis_fs)
    ax.set_ylabel("SRKW Recall at P90 Threshold", fontsize=axis_fs)
    ax.set_title(title, fontsize=title_fs, pad=8)
    ax.tick_params(labelsize=axis_fs)
    ax.grid(True, alpha=0.3)
    plt.rcParams['hatch.linewidth'] = prev

# ---------- inset panel acting as legend (Panel A) ----------
def _add_semicircle_size_panel(
    ax,
    levels=(0.2, 0.5, 0.8),
    size_method='linear', log_k=20.0,
    r_min=0.012, r_max=0.045, radius_scale=0.3,
    hatch_left='///', hatch_right='xxx', edge='0.35', lw=0.6,
    title="MCC (size + decision)",
    # [x0, y0, width, height] in axes fraction units
    box=(0.03, 0.64, 0.28, 0.30),
    font=11, title_fs=12, frame_edge='0.7',
    legend_size_ref=1.0,
):
    """A fully controlled mini-axes that replaces plt.legend for MCC size/decision."""
    axin = ax.inset_axes(list(box))
    axin.set_xlim(0, 1); axin.set_ylim(0, 1)
    axin.set_xticks([]); axin.set_yticks([])
    for sp in axin.spines.values():
        sp.set_visible(True); sp.set_edgecolor(frame_edge)
    axin.set_facecolor('white')

    # Title (will never be overlapped)
    axin.text(0.06, 0.93, title, ha='left', va='top', fontsize=title_fs)

    # Scale bubble radii within the box while preserving plot-relative scaling
    max_r_abs = r_max * radius_scale
    base_r = 0.11 * legend_size_ref     # max bubble radius as a fraction of box height
    x_c    = 0.18                        # bubble center x
    x_txt  = 0.36                        # label x
    ys = np.linspace(0.76, 0.28, len(levels))  # row centers (top->bottom)

    for m, y in zip(levels, ys):
        s     = _scale_vals([m], size_method, log_k)[0]
        r_abs = (r_min + (r_max - r_min) * s) * radius_scale
        r     = base_r * (r_abs / max_r_abs)

        axin.add_patch(Wedge((x_c, y), r,  90, 270,
                             facecolor='#bfbfbf', edgecolor=edge, lw=lw, hatch=hatch_left))
        axin.add_patch(Wedge((x_c, y), r, -90,  90,
                             facecolor='#bfbfbf', edgecolor=edge, lw=lw, hatch=hatch_right))
        axin.text(x_txt, y, f"{m:.1f}", ha='left', va='center', fontsize=font)

    return axin

# ---------- main figure ----------
def plot_mcc_grid_1x2_patterns(
    dclde_metrics_list, malahat_metrics_list, common_model_labels,
    zoom='per-panel', pad=0.03, min_span=0.15,
    size_method='linear', log_k=20.0,
    r_min=0.012, r_max=0.045, radius_scale=0.3,
    legend_levels=(0.2, 0.5, 0.8),
    figsize=(16, 8), dpi=600, savepath=None,
    axis_fs=12, title_fs=14, legend_fs=12, legend_title_fs=12,
    hatch_tkw='///', hatch_hw='xxx', hatch_edge='0.35', hatch_lw=0.6,
    legend_size_ref=1.0,               # scales ONLY the inset legend bubbles
    size_panel_box=(0.03, 0.64, 0.28, 0.30)  # position of inset legend on Panel A
):
    _validate_metrics_list(dclde_metrics_list,  "dclde_metrics_list")
    _validate_metrics_list(malahat_metrics_list, "malahat_metrics_list")

    # consistent colors per model
    base_cycle = plt.rcParams['axes.prop_cycle'].by_key()['color']
    while len(base_cycle) < len(common_model_labels):
        base_cycle += base_cycle
    color_map = {lab: base_cycle[i] for i, lab in enumerate(common_model_labels)}

    # axis limits
    d_x, d_y = _collect_xy(dclde_metrics_list)
    m_x, m_y = _collect_xy(malahat_metrics_list)
    if zoom == 'union':
        xL, yL = _compute_limits(np.concatenate([d_x, m_x]), np.concatenate([d_y, m_y]), pad, min_span)
        xR, yR = xL, yL; sharex = sharey = True
    else:
        xL, yL = _compute_limits(d_x, d_y, pad, min_span)
        xR, yR = _compute_limits(m_x, m_y, pad, min_span); sharex = sharey = False

    fig, axes = plt.subplots(1, 2, figsize=figsize, sharex=sharex, sharey=sharey)

    # Panel A
    _plot_panel_patterns(
        axes[0], dclde_metrics_list, common_model_labels, color_map,
        "DCLDE",
        size_method, log_k, r_min, r_max, radius_scale,
        hatch_tkw=hatch_tkw, hatch_hw=hatch_hw, hatch_edge=hatch_edge, hatch_lw=hatch_lw,
        axis_fs=axis_fs, title_fs=title_fs
    )
    axes[0].set_xlim(*xL); axes[0].set_ylim(*yL)
    axes[0].text(0.02, 0.98, "A", transform=axes[0].transAxes,
                 ha="left", va="top", fontsize=title_fs, fontweight="bold")

    # Panel B
    _plot_panel_patterns(
        axes[1], malahat_metrics_list, common_model_labels, color_map,
        "Malahat",
        size_method, log_k, r_min, r_max, radius_scale,
        hatch_tkw=hatch_tkw, hatch_hw=hatch_hw, hatch_edge=hatch_edge, hatch_lw=hatch_lw,
        axis_fs=axis_fs, title_fs=title_fs
    )
    axes[1].set_xlim(*xR); axes[1].set_ylim(*yR)
    axes[1].text(0.02, 0.98, "B", transform=axes[1].transAxes,
                 ha="left", va="top", fontsize=title_fs, fontweight="bold")

    # Panel A: add the inset “legend panel” (title never collides; labels centered)
    _add_semicircle_size_panel(
        axes[0],
        levels=legend_levels,
        size_method=size_method, log_k=log_k,
        r_min=r_min, r_max=r_max, radius_scale=radius_scale,
        hatch_left=hatch_tkw, hatch_right=hatch_hw, edge=hatch_edge, lw=hatch_lw,
        title="TKW MCC, HW MCC",
        box=size_panel_box,
        font=legend_fs,
        title_fs=legend_title_fs,
        legend_size_ref=legend_size_ref
    )

    # Panel B: standard model-color legend
    model_handles = [Rectangle((0,0), 1, 1, facecolor=color_map[lab], edgecolor='black')
                     for lab in common_model_labels]
    axes[1].legend(model_handles, common_model_labels,
                   loc='lower right', frameon=True,
                   fontsize=legend_fs, title="Models",
...
#-------------------- usage example --------------------
common_labels = [f"BirdNET_{i:02d}" for i in range(1, 10)]
fig, axes = plot_mcc_grid_1x2_patterns(
    dclde_metrics_list=[metrics_DCLDE_01, metrics_DCLDE_02, metrics_DCLDE_03,
                        metrics_DCLDE_04, metrics_DCLDE_05, metrics_DCLDE_06,
                        metrics_DCLDE_07, metrics_DCLDE_08, metrics_DCLDE_09],
    malahat_metrics_list=[metrics_malahat_01, metrics_malahat_02, metrics_malahat_03,
    ...
    hatch_tkw='///', hatch_hw='xxx', hatch_edge='0.35', hatch_lw=0.6,
    legend_levels=(0.2, 0.5, 0.8),
    legend_size_ref=1.2,                  # scales bubbles in the inset panel only
    #size_panel_box=(0.04, 0.64, 0.28, 0.30),
    size_panel_box=(0.69, 0.03, 0.28, 0.30),
    figsize=(16, 8), dpi=600,
    savepath='MCC_grid_patterns_1x2_inset_legend.pdf'
)
...
def _norm_keys(d):
    """Return a case-insensitive view of a dict: keys uppercased."""
    return {str(k).upper(): v for k, v in d.items()}

def build_auc_table_from_globals(prefix="metrics_DCLDE_", n_models=9, save_csv=None):
    """
    Collects AUCs from globals: metrics like metrics_DCLDE_01['AUC']['SRKW'] etc.
    Returns a DataFrame with columns ['SRKW','TKW','HW'] in that order.
    """
    rows = []
    for i in range(1, n_models + 1):
        var_name = f"{prefix}{i:02d}"
        if var_name not in globals():
            # model missing -> all NaNs
            rows.append((var_name, np.nan, np.nan, np.nan))
            continue

        metrics = globals()[var_name]
        auc_block = metrics.get('AUC', {})
        auc_ci = _norm_keys(auc_block)

        srkw = auc_ci.get('SRKW', np.nan)
        tkw  = auc_ci.get('TKW',  np.nan)
        hw   = auc_ci.get('HW',   np.nan)  # catches 'hw' thanks to _norm_keys

        rows.append((var_name, srkw, tkw, hw))

    df = pd.DataFrame(rows, columns=['model', 'SRKW', 'TKW', 'HW']).set_index('model')
...
# --- Usage ---
auc_table = build_auc_table_from_globals(prefix="metrics_DCLDE_", n_models=9, save_csv=None)
print(auc_table)

auc_table = build_auc_table_from_globals(prefix="metrics_malahat_", n_models=9, save_csv=None)
print(auc_table)
